...
public class RightSidebarShellForm : Form
{
    ...
    protected RightSidebarShellForm()
    {
        Text = "Спортклуб";
        ...
        _lightThemeItem.Click += (_, _) => UiTheme.SetTheme(AppTheme.Light);
        _darkThemeItem.Click += (_, _) => UiTheme.SetTheme(AppTheme.Dark);
    ...
    protected virtual Form? CreateFormForNavKey(string key) => key switch
    {
        ...
        "subscriptions" => new SubscriptionListForm(),
        "visits" => new VisitListForm(),
        "sell" => new SellSubscriptionForm(),
        "register" => new RegisterVisitForm(),
        "expiring" => new ExpiringSubscriptionsForm(),
        "visit-stats" => new VisitStatisticsReportForm(),
