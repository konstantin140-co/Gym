...
internal class EntityCardControl : RoundedPanel, IThemedControl
{
    ...
    private readonly Button _primaryAction = new() { Text = "Р РµРґР°РєС‚РёСЂРѕРІР°С‚СЊ", Width = 130 };
    private readonly Button _secondaryAction = new() { Text = "РЈРґР°Р»РёС‚СЊ", Width = 100 };
    ...
    public Button PrimaryAction => _primaryAction;
    public Button SecondaryAction => _secondaryAction;
    ...
    public void Bind(int id, string title, string status, string avatarLetter, string details,
        string primaryText = "Р РµРґР°РєС‚РёСЂРѕРІР°С‚СЊ", string secondaryText = "РЈРґР°Р»РёС‚СЊ")
    {
        EntityId = id;
        _title.Text = title;
