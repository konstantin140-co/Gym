...
internal class SidebarControl : Panel, IThemedControl
{
    ...
    public SidebarControl()
    {
        ...
        AddNavItem(menu, "Абонементы", "subscriptions");
        AddNavItem(menu, "Посещения", "visits");

        AddSection(menu, "Операции");
        AddNavItem(menu, "Продажа абонемента", "sell");
        AddNavItem(menu, "Регистрация визита", "register");
        AddNavItem(menu, "Истекающие", "expiring");

        AddSection(menu, "Отчёты");
        AddNavItem(menu, "Статистика визитов", "visit-stats");
        AddNavItem(menu, "Должники", "debtors");
